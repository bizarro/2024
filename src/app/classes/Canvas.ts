...
import type { OGLRenderingContext, Pass, RenderTargetOptions } from 'ogl'
import { Camera, Color, Geometry, Mesh, Post, Program, Renderer, RenderTarget, Vec2 } from 'ogl'
...
// The WebGL2 constants used below don't exist on the WebGL1 typings — at
// runtime they are simply `undefined` on a WebGL1 context.
const gl2 = gl as OGLRenderingContext & WebGL2RenderingContext
...
function supportRenderTextureFormat(gl: OGLRenderingContext, internalFormat: GLenum, format: GLenum, type: GLenum) {
...
const halfFloat = gl.renderer.isWebgl2 ? gl2.HALF_FLOAT : gl.renderer.extensions.OES_texture_half_float.HALF_FLOAT_OES
const filtering = supportLinearFiltering ? gl.LINEAR : gl.NEAREST

let rgba: SupportedFormat | null
let rg: SupportedFormat | null
let r: SupportedFormat | null
...
const lastMouse = new Vec2() as Vec2 & { isInit?: boolean }
...
export class Canvas {
  ...
  updateMouse(event: MouseEvent | TouchEvent) {
    // The original handler mutates the event, deriving `x`/`y` from touches
    // or page coordinates when they aren't present.
    const e = event as unknown as { changedTouches?: TouchList; x?: number; y: number; pageX: number; pageY: number }
